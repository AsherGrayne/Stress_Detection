...
class SimulatedScreen extends StatefulWidget {
  const SimulatedScreen({super.key});

  @override
  State<SimulatedScreen> createState() => _SimulatedScreenState();
}

class _SimulatedScreenState extends State<SimulatedScreen> {
  ...
  Future<void> _pull() async {
    try {
      final s = await _api.fetchSimulatedLatest();
      if (!mounted) return;
      setState(() {
        _snapshot = s;
        _error = null;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Simulated telemetry'),
        actions: [
          IconButton(
            ...
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => const HistoryScreen(source: 'simulated'),
                ),
              );
            },
          ),
